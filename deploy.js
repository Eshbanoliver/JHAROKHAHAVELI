...
if (fs.existsSync(sshKeyPath)) {
  config.privateKey = fs.readFileSync(sshKeyPath);
} else {
  config.password = 'Jharokhahaveli@1234';
}
...
conn.on('ready', async () => {
  ...
  try {
    ...
    let targetDir = '';
    const dirsToCheck = [
      `${pwd}/domains/cyan-ibex-756678.hostingersite.com/public_html`,
      `${pwd}/domains/cyan-ibex-756678.hostingersite.com`,
      `${pwd}/public_html`,
      '/var/www/html'
    ];
    ...
    if (!targetDir) {
      ...
      try {
        ...
        if (domainsList.includes('cyan-ibex-756678.hostingersite.com')) {
          targetDir = `${pwd}/domains/cyan-ibex-756678.hostingersite.com/public_html`;
